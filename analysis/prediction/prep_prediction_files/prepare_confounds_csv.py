...
subject_ids = [f for f in os.listdir(output_root) if os.path.isdir(os.path.join(output_root, f))]
subject_ids.remove("failures")

confounds_root = "/cbica/projects/clinical_dmri_benchmark/data/confounds"

def find_qc_csv(folder, subid, run):
    pattern = os.path.join(folder, f"{subid}_ses-PNC1_*{run}_desc-ImageQC_dwi.csv")
    matching_files = glob.glob(pattern)
    
    if matching_files:
        return matching_files[0]  # Return the first match (assuming only one correct file per folder)
    else:
...
for subject_id in subject_ids:
    ...
    for i, run in enumerate(["run-01", "run-02"]):
        extended_root = os.path.join(output_root, subject_id, "ses-PNC1", "dwi")
...
head_motion_df = pd.DataFrame(head_motion_list, columns=["subject_id", "mean_fd_run-01", "max_fd_run-01", "mean_fd_run-02", "mean_fd_run-02"])
head_motion_df.to_csv(os.path.join(confounds_root, "head_motion.csv"), index=False)
...
demographics = pd.read_csv(os.path.join(confounds_root, "n1601_demographics_go1_20161212.csv"))
tbv = pd.read_csv(os.path.join(confounds_root, "n1601_ctVol20170412.csv"))[["bblid", "mprage_antsCT_vol_TBV"]]
id_conversion_file = pd.read_csv("/cbica/projects/clinical_dmri_benchmark/data/QC/bblid_scanid_sub.csv")
