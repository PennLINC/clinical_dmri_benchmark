...
def get_completed_subjects(qsiprep_outputs: str, available_subjects: list):
    ...
    if os.path.exists(qsiprep_outputs) == False:
        logging.info("No output folder found at " + qsiprep_outputs +
                     ". Assuming no subjects have been processed so far.")
        return []

    processed_subIDs = []
    for subject in available_subjects:
        subject_in_outputs = os.path.exists(
            os.path.join(qsiprep_outputs, subject))
        subject_in_failures = os.path.exists(
            os.path.join(qsiprep_outputs, 'failures', subject))
...
def get_available_subjects(data_dir: str):
    ...
    data_dir_folders = [
        entry for entry in data_dir_content if not os.path.isdir(entry)]
    subject_folders = [
        folder for folder in data_dir_folders if folder.startswith('sub-')]
...
if __name__ == '__main__':
    ...
    completed_subjects = get_completed_subjects(
        OUTPUTS_QSIPREP, subjects_to_process)

    needs_processing = sorted(
        set(subjects_to_process) - set(completed_subjects))
