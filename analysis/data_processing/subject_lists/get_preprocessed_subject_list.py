...
parser.add_argument('--recon_suffix', type=str, required=True,
...
def get_reconstructed_subjects(qsirecon_outputs: str, preprocessed_subjects: list, qsirecon_suffix: str):
    ...
    """
    qsirecon_suffix_options = ['GQIautotrack',
                               'SS3Tautotrack', 'CSDautotrack', 'SS3T', 'CSD']
    ...
    if os.path.exists(full_path_qsirecon_outputs) == False:
        logging.info("No output folder found at " + full_path_qsirecon_outputs +
                     ". Assuming no subjects have been reconstructed so far.")
        return []

    reconstructed_subIDs = []
    for subject in available_subjects:
        subject_in_outputs = os.path.exists(
            os.path.join(full_path_qsirecon_outputs, subject))
        subject_in_failures = os.path.exists(os.path.join(
            full_path_qsirecon_outputs, 'failures', subject))
...
preprocessed_subjects = get_completed_subjects(
    OUTPUTS_QSIPREP, available_subjects)
reconstructed_subjects = get_reconstructed_subjects(
    OUTPUTS_QSIRECON, preprocessed_subjects, QSIRECON_SUFFIX)

needs_reconstruction = sorted(
    set(preprocessed_subjects) - set(reconstructed_subjects))

logging.info(
    f"Found {len(needs_reconstruction)} sessions that have been successfully pre-processed and not yet reconstructed using {QSIRECON_SUFFIX}.")
