...
RECONSTRUCTION_OUTPUTS = Path("/cbica/projects/clinical_dmri_benchmark/results/qsirecon_outputs/qsirecon-GQIautotrack")
...
failed_subject_ids = [f.name for f in failed_reconstructions.iterdir() if f.is_dir()]
logging.info(f"Found {len(failed_subject_ids)} subjects that reconstruction failed for.")
...
reconstructed_subject_ids = [f.name for f in RECONSTRUCTION_OUTPUTS.iterdir() if f.is_dir()]
reconstructed_subject_ids.remove("failures")
acq_variant_counter = 0
for reconstructed_subject in reconstructed_subject_ids:
    subject_path = RECONSTRUCTION_OUTPUTS / reconstructed_subject / "ses-PNC1" / "dwi"
    dwimap_file = [f for f in subject_path.iterdir() if f.is_file() and "run-01_space-T1w_dwimap.fib" in f.name][0].name
...
logging.info(f"Found {acq_variant_counter} subjects that were acquired with an acquisition variant.")
...
failed_qc_counter = 0
excluded_subjects_qc = list(df_qc[df_qc["dti64Exclude"] == 1]["subject_id"].values)
