...
for tract_name in tract_names:
    ...
    union = np.logical_or.reduce([prob_map_gqi != 0, prob_map_csd != 0, prob_map_ss3t != 0, atlas_mask != 0])
    for subid in subids:
        for run in runs:
            mask_name = subid + "_ses-PNC1_" + run + "_space-MNI152NLin2009cAsym_bundle-" + tract_name_short + "_mask.nii.gz"
            mask_path = os.path.join(subject_masks_path, subid, "ses-PNC1", "dwi", "MNI", mask_name)
            if os.path.exists(mask_path):
                mask = sitk.ReadImage(mask_path, sitk.sitkUInt8)
                sensitivity, specificity = compute_sensitivity_specificity(mask, atlas_mask, union)
